...
$PORowId=$_GET['PoRID'];	
	...
	$sqlGetOrderData="SELECT `custCode`, `projectName`, `PoNum` FROM `customerpo`, `job` 
	WHERE `jobidref` = `jobId` AND `poId` = $PORowId";
	...
	$sqlGetDocNum="SELECT `docSerial` FROM `warehouse` WHERE `poIdRef` = $PORowId  
...
<title>Export Stock Docs Num&nbsp;From Customer :&nbsp;<?php echo $CustName;
  ...
  <tr>
    ...
    <div class="landscaep">
     <center>
    	...
    	<p><b><span style="font-size:22px">Export Stock Document/s</span>
...
<table class="table" height="auto" style="border:solid; border:1px">
	...
	$sqlFetchNewStock="SELECT `warehouseId`, `description`, `export` FROM `warehouse` WHERE 
	`poIdRef` = $PORowId"; 
