           	...
           	$.ajax({
				...
				success: function(doneExportedAutoOffer)
				{
					if(doneExportedAutoOffer == 1)
						{
						var newAutoDocPrint = window.open("dist/php/printAutoOffer.php?&JobId="+jobRowIDExportAuto,"_balnk");							
						setTimeout(function(){
...
<?php 
   $sqlGetAutoDooes = "SELECT `id`, `doortype`, `doorspecs`, `motorspecs`, `doorprice`,
	 `doorqty`, `totalprice` FROM `autodoorsoffer` WHERE  `jobid` = $jobRowIdExp";
...
while($resGetAutoDooes = mysqli_fetch_assoc($queryGetAutoDooes))
{
	echo "
		<tr>
			<td class='col-sm-2'>$resGetAutoDooes[doortype]</td>
			<td class='col-sm-3'>$resGetAutoDooes[doorspecs]</td>
			<td class='col-sm-3'>$resGetAutoDooes[motorspecs]</td>
			<td class='col-sm-1'>$resGetAutoDooes[doorqty]</td>
			<td class='col-sm-1'>".number_format($resGetAutoDooes['doorprice'])."</td>
