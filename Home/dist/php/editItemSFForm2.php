...
date_default_timezone_set('Africa/Cairo');
 ...
 $sqlGetItems="SELECT `descriptionname`, `salesfactor`, `overprice`, `description` FROM `stockitems` 
...
?>
 <div class="modal-header">
        <h5 class="modal-title">Edit Sales Factor & Overcost for Item :&nbsp;<span style="color:blue;"><?php echo $resGetItems['descriptionname'];?></span></h5>
...
<table class="table table-sm " style="width:100%" align="center">
      ...
      <tbody>
      <td> <input type="text" readonly class="form-control"  step="0.01"
	  value="<?php echo number_format($resGetItemCost['cost']); ?>" /></td>
      <td><input type="text" readonly class="form-control" 
	  value="<?php echo number_format($resGetItemCost['sales']); ?>" /></td>
		<td >
        	<div class="input-group">
              <input type="number" class="form-control itemSalesFact" id="itemSalesFact" aria-label="%" 
              list="presntageVal" min="1" value="<?php echo ($resGetItems['salesfactor'] * 100);?>">
        ...
        <td >
         <input type="number" class="form-control itemOverCost" id="itemOverCost" min="0" 
         value="<?php echo $resGetItems['overprice'];?>"/>
        </td>
 <tr>
 	<td colspan="4" align="center">	
    	<button class="btn btn-sm btn-success" id="saveEditSalseFact">Save</button>
    </td> 
  </tr>
  </tbody>
 ...
 $(document).ready(function() {
	...
	$("#saveEditSalseFact").click(function(){
		...
		else
		{
			...
			$.ajax({
					...
					url:"dist/php/saveEditSFactor.php",
					type:"POST",
					data:{IRID:ItemRID, SalesFactVal:SalesFactPeg, overPriCost:itemOverCost},
					beforeSend: function(){
						$("#saveEditSalseFact").prop('disabled', true);	
					},
					success: function(doneAddSalesFact){
						...
						if(doneAddSalesFact == 1)
						{
							alert("Date Saved");
							 ...
							 $(".itemHaveSF").html("");
							 $(".itemHaveSF").load("dist/php/showAllItemsForEditSF.php");
						...
						else
						{
							...
							$("#saveEditSalseFact").prop('disabled', false);	
						}
