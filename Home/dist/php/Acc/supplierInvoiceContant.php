     ...
     <?php
      ...
      while($orderItemsData=mysqli_fetch_assoc($querySupplierOrderItemsData)){
       ...
       # Stock Type
        ...
        $sqlOrderItems="SELECT `id`, `descripcode`,`descripqty`,`descripprice`,`totalprice`,`jobref`,`ref`,`whref`, `purchasesCost` FROM `stockoffers` WHERE `id` = $itemRowId";
        $queryOrderItemsData=mysqli_query($link,$sqlOrderItems)or die("ERROR_SNSC : 01");
        $itemsData=mysqli_fetch_assoc($queryOrderItemsData);
       ...
       # Maintenance Type
        ...
        $sqlOrderItems="SELECT `id`, `type`, `hights`, `widths`, `depths`, `price`, `typeqty`, `totalprice`, `jobid`, `ref`, `purchasesCost` FROM `maintoffers` WHERE `id` = $itemRowId";
        $queryOrderItemsData=mysqli_query($link,$sqlOrderItems)or die("ERROR_SNSC : 01");
        $itemsData=mysqli_fetch_assoc($queryOrderItemsData);
...
<script type="text/javascript">
 $(document).ready(function(){
  ...
  $("#saveInvoice").click(function(){
   ...
   $.ajax({
    url:'dist/php/Acc/saveSupplierInvoice.php',
    type:"POST",
 ...
 $("#printPo").click(function(){
  var rowId = $(this).val();
  $.ajax({
   url:'dist/php/Acc/printManufacturOrder.php',
   type:"POST",
   data:{orderId:rowId},
   success: function(invSuuplierData2){
window.open("dist/php/Acc/printManufacturOrder.php?orderId="+rowId, "_blank");
   }
  });
