...
customize: function ( win ) {
  ...
  "footerCallback": function(row, data, start, end, display){
   ...
   $(".showAllOfferInfo").click(function(){
	   ...
	   var PageRef = $("#pageRef").val();
	   
	   if(JobType == "Doors")
	   {
	   
	   	$('.allOffersHist').html("");
		$('.allOffersHist').load("dist/php/WonOffersDetialsD.php",{jobRId:offerRID, backRef:PageRef});
	   }
	   else if(JobType == "Automatic")
	   {
	   
	   	$('.allOffersHist').html("");
		$('.allOffersHist').load("dist/php/WonOffersDetialsA.php",{jobRId:offerRID, backRef:PageRef});
	   }
	   else if(JobType == "Stock")
	   {
	   
	   	$('.allOffersHist').html("");
		$('.allOffersHist').load("dist/php/WonOffersDetialsS.php",{jobRId:offerRID, backRef:PageRef});
	   } 
	   else if(JobType == "Maintenance")
	   {
	   
	   	$('.allOffersHist').html("");
		$('.allOffersHist').load("dist/php/WonOffersDetialsM.php",{jobRId:offerRID, backRef:PageRef});
	   } 
	   ...
	   $(".PrintOldOffer").click(function(){
		...
		$(".returnToEdit").click(function(){
		    ...
		    var offerRID3 = $(this).val();
			 var PageRef2 = $("#pageRef").val();
	   		
			var confirmReturn = confirm("Please Confirm Return job to edit?");
			
			if(confirmReturn === true)
			{
				$.ajax({
					
					url:"dist/php/saveReturnJob.php",
					type:"POST",
					data:{jobRIdR:offerRID3},
					beforeSend: function(){
					$('.returnToEdit').prop('disabled', true);	
					},
					success: function(doneReturnJob){
						
						if(doneReturnJob == 1)
						{
							alert("Date Saved.. ");
							$('.allOffersHist').html("");
							$('.allOffersHist').load("dist/php/allWonOffersHistory.php",{RPTRef:PageRef2});
						}
						else
						{
							alert(doneReturnJob);
							$('.returnToEdit').prop('disabled', false);
						}
...
<div class="table-responsive-sm">
 <table class="myTableWon table table-sm table-striped table-bordered" cellspacing="0" width="99%">
...
<?php
 ...
 include_once("connection.php");
$ser = 1;
	$sqlGetAllNewJob="SELECT `jobId`, `startDate`, `localref`, `projectName`, `customer`, `offerValue`,
	 `responsible`, `offerStatus`,	`jobtype`, `description`, `jobtype`, `salesman`, `lastupdate`, `poref`
	 , `endDate` FROM `job` WHERE `jobref` = $refrance";
	...
	$resGetSales= mysqli_fetch_assoc($queryGetSales);
		...
		echo "
			<tr>
				...
				<td data-toggle='tooltip' data-placement='left' title='$resGetAllNewJob[localref]'>
				$resGetAllNewJob[projectName]</td>
				...
				<td style='color:blue'>".number_format(($resGetAllNewJob['offerValue']) ,1)."</td>
				...
				title='Return to Edit'>
					<button class='btn btn-xm btn-link returnToEdit' 
					value='$resGetAllNewJob[jobId]'>
					<i class='fa fa-undo' aria-hidden='true' style='font-size:18px;color:#0275d8'></i>
					</button>
				</span></td>
            </tr>
