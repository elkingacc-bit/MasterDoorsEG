...
$rowIDItem = $_POST['TRIDDoor'];
	...
	$sqlGetItemRef = "SELECT `type`, `totalprice` FROM `maintoffers` WHERE `id` = $rowIDItem ";
...
$offerVal = ($offerVal - $itemTotalPrice);
 ...
 mysqli_query($link,$sqlUpdateOfferStatus)or die("ERROR :05-ANJ_GCN_S");
	...
	$DoorType = $resGetItemRef['type'];
	
	$sqlRemoveItem = "DELETE FROM `maintoffers` WHERE `id` = $rowIDItem
	LIMIT 1";
	mysqli_query($link,$sqlRemoveItem)or die("ERROR :02-ANJ_GCN_S");
	
	
	$action="Remove Type : $DoorType From Job Number : $jobLocalRef";
$logRef=5;	
