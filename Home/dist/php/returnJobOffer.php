...
$jobLocalRef = $resGetJobNum['localref'];
 $sqlUpdateOfferStatus = "UPDATE `job` SET `jobref` = NULL , `offerStatus` = 'Proccessing' 
 WHERE `jobId` = $jobRowId";
 mysqli_query($link,$sqlUpdateOfferStatus)or die("ERROR :02-ANJ_GCN_S");
 
 $sqlUpdateOfferProp = "UPDATE `offerproperties` SET `jobproref` = 0 WHERE `jobidref` = $jobRowId";
 mysqli_query($link,$sqlUpdateOfferProp)or die("ERROR :03-ANJ_GCN_S");
 
 $sqlUpdateOfferItems = "UPDATE `itemoffer` SET `ref` = 0 WHERE `jobref` = $jobRowId";
 mysqli_query($link,$sqlUpdateOfferItems)or die("ERROR :04-ANJ_GCN_S");
	
	$action="Confrim Offer for Job Number : $jobLocalRef";
