...
$AsKitRID = $_POST['unitAddRowID'];

	$sqlGetKitName = "SELECT `kitName` FROM `assemblykits` WHERE `id` = $AsKitRID";
...
<input type="number" id="kitId" value="<?php echo $AsKitRID;?>" style="display:none" />
          ...
          <tr>
          		<td>
					<input type='text' id='partNo' name='partNo' class='form-control' 
                     autocomplete='off' style='font-weight:bold' value='' 
                     list="AllPartNum"/>
                ...
                class='img-thumbnail img-AddItemAk' style='cursor: zoom-in;'/>
                </td>
          </tr>
          <tr>
            <td align="center" colspan="4">
            	<button class="btn btn-success btn-sm" id="addUnitBTN">save</button>
            </td>  	
          </tr>
        </tbody>
    </table>
...
else if(selectedDescrip == "")
{
	...
	else
	{
		$.ajax({
				...
				url:"dist/php/saveAddUnitToAsKit.php",
				type:"POST",
				data:{partNo:partNumber, descCode:Description, descName:DecripCode,AsKitQty:requierQTY, AsKitName:AsKName, AsKitRowID: AsKIdR},
				beforeSend: function(){
				$("#addUnitBTN").prop('disabled', true);	
				},
				success: function(doneAddItemToAsKit){
					
					if(doneAddItemToAsKit == 1)
					{
						alert("Data Saved");
						$(".ShowEditForm").html("");
					...
					else if(doneAddItemToAsKit == 0)
					{
						alert("Item is already exist in this Assembly Kit Componant");
						$("#ItemName").css("border-color","red");
