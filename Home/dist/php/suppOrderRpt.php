...
customize: function ( win ) {
    ...
    .css( {'font-size' :'inherit',  'text-align': 'left'} );
		...
		$(".editSOBiscData").click(function(){
		
		var SupporderRowIdEdit = $(this).val();
		...
		//$(".BackFromEdit").show();
			...
			$.ajax({
                url:'dist/php/showEditSuppOrderFormModel.php',
                type:'POST',
                data:{ORIDEdit:SupporderRowIdEdit},
                
				success: function(showSuppOrderDateForm)
				{
				//alert(showHWData);
                $('.ShowData').html('');
                $('.ShowData').html(showSuppOrderDateForm);
				$(".myModal").modal('toggle');
...
$ser = 1;
	$sqlGetAllNewSuppOrder="SELECT `SOId`, `SuppCode`, `OrderNumber`, `date`, `deliveryDate`, `totalAmout`,
	 `authUser`, `orderNotes`, `custPOId`, DATEDIFF(`deliveryDate`, NOW()) AS Deff , `SORef` FROM `supplierorder`";
	...
	while($resGetAllSuppOrder= mysqli_fetch_assoc($queryGetAllSuppOrder))
	{
	    ...
	    if($resGetAllSuppOrder['SORef'] == 0)
	    {
	        $status = 'New';
	    }
	    else if($resGetAllSuppOrder['SORef'] == 1 || $resGetAllSuppOrder['SORef'] == 2)
	    {
	        $status = 'run';
	    }
	    else if($resGetAllSuppOrder['SORef'] == 3 )
	    {
	        $status = 'end';
	    }
	    else
	...
	if(mysqli_num_rows($queryGetCustPO) > 0)
	{
	...
	else
	{
	  ...
	  if($resGetAllSuppOrder['SORef'] == 3)
	    {
	        $status = 'End';
	        $remPo = $resGetPolicy['Deff'];
	        $remSuppO = 0;
	    }
	    else
	    {
	       $remPo = $resGetPolicy['Deff'];
	        $remSuppO =$resGetAllSuppOrder['Deff'];
	    }
		...
		echo "
			<tr>
				...
				<button class='btn btn-link editSOBiscData' value='$resGetAllSuppOrder[SOId]' 
				$action>$ser</button>
				</span></td>
				...
				<td>$receiceQTYTD</td>
				<td >$suppReminig</td>
				<td style='color:$color'>$remPo</td>
				<td style='color:$SuppColor'> $remSuppO</td>
            </tr>
