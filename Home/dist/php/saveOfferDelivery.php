...
date_default_timezone_set('Africa/Cairo');
 ...
 {
 //echo "test --> ";
 $JobRowId = $_POST['DelvJobRowId'];
 $dDate = $_POST['Delvdate'];
 $dNote = nl2br($_POST['DelvNotes']);
 $dNote = trim($dNote);
	...
	$sqlAttName = "UPDATE `offerpolicy` SET `deliver` = '$dNote', `deliverydate`= '$dDate' 
	WHERE `jobRowId` = $JobRowId";
	mysqli_query($link,$sqlAttName)or die("ERROR :04-AU_AU_S");
	
	
	$action="Add Delivery trems end Date : $dDate for Customer: $custName Job No: $jobLocalRef";
$logRef=5;	
