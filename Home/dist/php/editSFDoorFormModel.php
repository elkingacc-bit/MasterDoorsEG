...
if($jobTypeSF1 == 'Doors')
{
	$sqlGetm2Price = "SELECT `msquerprice`, `shipping`, `installation`, `margin` FROM `itemoffer` 
	WHERE `jobref` = $JobRowId";
	$queryGetm2Price = mysqli_query($link,$sqlGetm2Price)or die("ERROR :01-ANJ_GCN_S");
	if(mysqli_num_rows($queryGetm2Price) > 0)
	{
		$resGetm2Price = mysqli_fetch_assoc($queryGetm2Price);
	
		echo $Price = $resGetm2Price['msquerprice'];
		echo $Shapping = $resGetm2Price['shipping'];
		echo $Install = $resGetm2Price['installation'];
		echo $Overhead = round($resGetm2Price['margin'] * 100);
	}
	else
	{
       ...
       <div class="modal-body ">
       	<table class="table table-sm">
        	...
        	<tr>
                ...
                <td>
                	<input type="number" class="form-control Install" min="0" step="0.01" 
                    autocomplete="off" value="<?php echo $Install;?>"/>
                </td>
                <td>
                	<input type="number" class="form-control Sipp" min="0" step="0.01"  
                    value="<?php echo $Shapping;?>" autocomplete="off">
                </td>
                <td >
                <div class="input-group">
                  <input type="number" class="form-control hwOverhead" id="hwOverhead" aria-label="%" 
                  list="hwOverheadList" min="1" autocomplete="off" value='<?php echo $Overhead;?>'>
